...
export const handleDeleteLinkBon_PDF = async (pdfId: number, modalId: string, entreprise_id: string | null) => {

    //Message pour demander confirmation de suppression
    const confirm = await Swal.fire({
    title: "Supprimer le lien Bon PDF-BSD",
    text: "Voulez-vous vraiment supprimer le lien entre le Bon PDF et le BSD ?",
    icon: 'warning',
    ...
    // 1. Récupérer l'ID du bon_pdf et le linked_bsd_id
    ...
    if (!bonPdfData.linked_bsd_id) throw new Error('Aucun BSD lié à ce Bon PDF');

    const linkedBsdId = bonPdfData.linked_bsd_id;
    ...
    // 4. Mettre à jour le BSD : supprimer bon_extracted_then_linked_id et mettre à jour pdf_ids
    const { error: updateBsdError } = await supabase
      .from('bsd')
      .update({
        bon_extracted_then_linked_id: null,
        pdf_ids: newPdfIds
      })
      .eq('id', linkedBsdId)
      .eq('entreprise_id', entreprise_id);
    ...
    // 5. Supprimer le linked_bsd_id du bon_pdf
    ...
    toast.success('Lien Bon PDF-BSD supprimé avec succès');
