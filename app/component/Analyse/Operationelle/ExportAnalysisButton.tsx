...
const ExportAnalysisButton = () => {
    ...
    const handleExport = async () => {
        if (entreprise_id) {
            setIsLoading(true);
            try {
                const response = await fetch(`/api/analysis/export_analysis_excel?entreprise_id=${entreprise_id}`);
                
                if (response.ok) {
                    const filename = response.headers.get('Content-Disposition')?.split('filename=')[1]?.replace(/"/g, '') || 'analyse_bsd.xlsx';
        ...
        <div className="flex items-center space-x-2">
            ...
            >
                {isLoading ? (
                    ...
                    <>
                        <BoxIcon name="download" type="solid" size="sm" />
                        <span className="text-sm">Récap Comparatif Demandes/Imports</span>
                    </>
                )}
            </button>
